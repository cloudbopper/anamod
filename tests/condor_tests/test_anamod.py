...
def test_condor_simulation_interactions(file_regression, tmpdir):
    ...
    cmd = ("python -m anamod.simulation -condor -seed 5 -num_instances 100 -num_features 10 -fraction_relevant_features 0.5"
...
def test_condor_simulation_noisy_interactions(file_regression, tmpdir):
...
def test_condor_simulation_shuffling_interactions(file_regression, tmpdir):
    """Test simulation with interactions and shuffling perturbations"""
    func_name = sys._getframe().f_code.co_name
    output_dir = "%s/output_dir_%s" % (tmpdir, func_name)
    pvalues_filename = "%s/%s" % (output_dir, constants.INTERACTIONS_PVALUES_FILENAME)
    cmd = ("python -m anamod.simulation -condor -seed 5 -num_instances 1000 -num_features 10 -fraction_relevant_features 0.5"
