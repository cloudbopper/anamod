...
def test_condor_simulation_random_hierarchy(file_regression, tmpdir, caplog, shared_fs):
    ...
    cmd = ("python -m anamod.simulation -condor 1 -memory_requirement 1 -disk_requirement 1"
           ...
           " -analysis_type hierarchical -contiguous_node_names 1 -hierarchy_type random -perturbation zeroing -cleanup 0"
...
def test_condor_simulation_clustering_hierarchy(file_regression, tmpdir, caplog, shared_fs):
    ...
    cmd = ("python -m anamod.simulation -condor 1 -memory_requirement 1 -disk_requirement 1"
           ...
           " -analysis_type hierarchical -contiguous_node_names 1 -hierarchy_type cluster_from_data -perturbation zeroing -cleanup 0"
           f" -shared_filesystem {shared_fs} -output_dir {output_dir}")
    pass_args = cmd.split()[2:]
    with patch.object(sys, 'argv', pass_args):
        simulation.main()
...
def test_condor_simulation_gaussian_noise(file_regression, tmpdir, caplog, shared_fs):
    """Test simulation with clustering hierarchy"""
    func_name = sys._getframe().f_code.co_name
    output_dir = pre_test(func_name, tmpdir, caplog)
    cmd = ("python -m anamod.simulation -condor 1 -memory_requirement 1 -disk_requirement 1"
           " -seed 4 -num_instances 100 -num_features 30 -fraction_relevant_features 0.5 -analysis_type hierarchical"
