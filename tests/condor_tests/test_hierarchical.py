...
def test_condor_simulation_random_hierarchy(file_regression, tmpdir, caplog):
    ...
    cmd = ("python -m anamod.simulation -condor 1 -seed 1 -num_instances 100 -num_features 10 -fraction_relevant_features 0.5"
...
def test_condor_simulation_clustering_hierarchy(file_regression, tmpdir, caplog):
    ...
    func_name = sys._getframe().f_code.co_name
    output_dir = pre_test(func_name, tmpdir, caplog)
    cmd = ("python -m anamod.simulation -condor 1 -seed 2 -num_instances 100 -num_features 10 -fraction_relevant_features 0.5"
...
def test_condor_simulation_shuffling_perturbation(file_regression, tmpdir, caplog):
    ...
    func_name = sys._getframe().f_code.co_name
    output_dir = pre_test(func_name, tmpdir, caplog)
    cmd = ("python -m anamod.simulation -condor 1 -seed 3 -num_instances 100 -num_features 10 -fraction_relevant_features 0.5"
           " -contiguous_node_names 1 -hierarchy_type random -perturbation shuffling -num_shuffling_trials 10 -output_dir %s" % output_dir)
    pass_args = cmd.split()[2:]
    with patch.object(sys, 'argv', pass_args):
        simulation.main()
    post_test(file_regression, caplog, output_dir)
...
def test_condor_simulation_gaussian_noise(file_regression, tmpdir, caplog):
    ...
    func_name = sys._getframe().f_code.co_name
    output_dir = pre_test(func_name, tmpdir, caplog)
    cmd = ("python -m anamod.simulation -condor 1 -seed 4 -num_instances 100 -num_features 10 -fraction_relevant_features 0.5"
...
def test_condor_simulation_interactions(file_regression, tmpdir, caplog):
    ...
    func_name = sys._getframe().f_code.co_name
    output_dir = pre_test(func_name, tmpdir, caplog)
    cmd = ("python -m anamod.simulation -condor 1 -seed 9 -num_instances 100 -num_features 10 -fraction_relevant_features 0.5"
...
def test_condor_simulation_all_pairwise_interactions(file_regression, tmpdir, caplog):
    ...
    cmd = ("python -m anamod.simulation -condor 1 -seed 9 -num_instances 100 -num_features 10 -fraction_relevant_features 0.5"
           ...
           " -num_interactions 3 -output_dir %s -analyze_all_pairwise_interactions 1" % output_dir)
    pass_args = cmd.split()[2:]
    with patch.object(sys, 'argv', pass_args):
        simulation.main()
    post_test(file_regression, caplog, output_dir, interactions=True)
