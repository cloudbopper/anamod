...
def test_condor_simulation_regressor1(data_regression, tmpdir, caplog):
    ...
    cmd = ("python -m anamod.simulation -condor 1 -memory_requirement 1 -disk_requirement 1 -seed 100 -analysis_type temporal -num_instances 100 -num_features 10 "
...
def test_condor_simulation_classifier1(data_regression, tmpdir, caplog):
    """Test simulation with random hierarchy"""
    func_name = sys._getframe().f_code.co_name
    output_dir = pre_test(func_name, tmpdir, caplog)
    cmd = ("python -m anamod.simulation -condor 1 -memory_requirement 1 -disk_requirement 1 -seed 100 -analysis_type temporal -num_instances 100 -num_features 10 "
