...
from anamod.baselines.run_explainer import EXPLAINERS, TRUE_SCORES_FILENAME, EXPLAINER_SCORES_FILENAME, EXPLAINER_RUNTIME_FILENAME
...
def main():
    ...
    parser.add_argument("-num_repeats", type=int, default=2, help="number of repeats to perform and average results over")
    parser.add_argument("-start_seed", type=int, default=100000, help="randomization seed for first repeat, incremented for"
                        " every subsequent repeat.")
    parser.add_argument("-condor", type=strtobool, default=True, help="Flag to enable condor for running repeats in parallel"
    ...
    parser.add_argument("-repeat_missing", type=strtobool, default=False,
                        help="Only run explainer for repeats where results are missing (due to failure/abort)")
...
def explain(args, sconfig):
    ...
    jobs = []
    for ridx in range(args.num_repeats):
        job_dir = f"{args.output_dir}/{ridx}"
        if args.repeat_missing and os.path.exists(f"{job_dir}/{EXPLAINER_SCORES_FILENAME}"):
            continue
        seed = args.start_seed + ridx
        cmd = (f"python -m anamod.baselines.run_explainer -explainer {args.explainer} -model {args.model} -model_type {args.model_type}"
...
def evaluate(args):
    ...
    power = np.zeros(args.num_repeats)
    fdr = np.zeros(args.num_repeats)
    runtime = np.zeros((args.num_repeats, 2))
    for ridx in range(args.num_repeats):
        # Load results
